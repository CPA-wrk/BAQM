...
# #
# test_that("lm_plot.qq plot attributes for cars data are ok", {
#   withr::local_options(test_options)
#   expect_equal(
#     (lm_plot.qq_cars.new <- attributes(lm_plot.qq_cars)),
#     (lm_plot.qq_cars.old <- readRDS(
#       file = file.path(fixt_path, "lm_plot.qq_cars.rds")))
#   )
# })
#
test_that("lm_plot.qq plot graphics for cars data are ok", {
  withr::local_options(test_options)
  vdiffr::expect_doppelganger(
    title = "lm-plot-qq-cars",
    fig = lm_plot.qq_cars_bld(test_options),
    variant = sysnm())
})
