...
test_that("lm_plot.infl produces expected plot elements for cars", {
  ...
  lm_plot.infl_cars.out <- list()
  nms <- names(lm_plot.infl_cars)
  for (nm in nms[!nms %in% c("mdl", "plts", "p_4way")]) {
    lm_plot.infl_cars.out[[nm]] <-
      testthat::capture_output_lines(print(lm_plot.infl_cars[[nm]]))
  }
  # lm_plot.infl_cars.txt <- lm_plot.infl_cars.out
  # dump("lm_plot.infl_cars.txt", file = "chk/lm_plot.infl_cars.txt")
  lm_plot.infl_cars.txt <- list(
    opt = "list()",
    ...
    df = c("                                    .id .sequence .obs    .fits     .resid",
           ...
           "Volvo 142E              reg"))
  expect_equal(lm_plot.infl_cars.out, lm_plot.infl_cars.txt)
})
