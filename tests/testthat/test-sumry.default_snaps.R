# Testing printed output for sumry.default
...
test_that("Check printed results: mtcars numeric only", {
  ...
  expect_snapshot(sumry(mtcars, pad = 1))
})
# test_that("Check printed results: mtcars numeric only", {
#   withr::local_options(test_options)
...
#     sumry(mtcars, pad = 1),
#     print = FALSE,
#     width = test_options$width)
#   rslt <- expect_equal(df.out, c(
...
test_that("Check printed results: penguins mixed types, and transpose", {
  withr::local_options(test_options)
  expect_snapshot(sumry(penguins, pad = 1, transpose = TRUE))
})
#
# test_that("Check printed results: penguins mixed types, and transpose", {
#   withr::local_options(test_options)
...
test_that("Check printed results: vector, and transpose", {
  withr::local_options(test_options)
  expect_snapshot(sumry(1:10, transpose = TRUE))
})
#
# test_that("Check printed results: vector, and transpose", {
#   withr::local_options(test_options)
#   expect_equal(testthat::capture_output_lines(
