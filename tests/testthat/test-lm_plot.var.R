...
test_that("lm_plot.var produces expected plot elements for cars", {
  ...
  lm_plot.var_cars.out <- list()
  nms <- names(lm_plot.var_cars)
  for (nm in nms[!nms %in% c("mdl", "plts", "p_4way")]) {
    lm_plot.var_cars.out[[nm]] <-
      testthat::capture_output_lines(print(lm_plot.var_cars[[nm]]))
  }
  # lm_plot.var_cars.txt <- lm_plot.var_cars.out
  # dump("lm_plot.var_cars.txt", file = "chk/lm_plot.var_cars.txt")
  lm_plot.var_cars.txt <- list(
    opt = c("$pval.BP", "[1] TRUE", ""),
    parm = c("$pts", "$pts$symsz",
             ...
             "", "$xtrem$outl$thrshld", "[1] 1.5", "", "", "", "$var", "$var$lim",
             "          x        y", "min 10.2773 -5.12653", "max 28.0268  6.98840",
             "", "$var$BP", "", "\tstudentized Breusch-Pagan test", "", "data:  mdl",
             "BP = 2.415, df = 2, p-value = 0.299", "", "", ""),
    df = c("                                    .id .sequence .obs   .fits     .resid",
           ...
           "Volvo 142E              reg"))
  expect_equal(lm_plot.var_cars.out, lm_plot.var_cars.txt)
})
