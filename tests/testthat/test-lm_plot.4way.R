...
test_that("lm_plot.4way produces expected plot content for cars", {
  ...
  lm_plot.4way_cars.out <- list()
  nms <- names(lm_plot.4way_cars)
  for (nm in nms[!nms %in% c("mdl", "plts", "p_4way")]) {
    lm_plot.4way_cars.out[[nm]] <-
      testthat::capture_output_lines(print(lm_plot.4way_cars[[nm]]))
  }
  # lm_plot.4way_cars.txt <- lm_plot.4way_cars.out
  # dump("lm_plot.4way_cars.txt", file = "chk/lm_plot.4way_cars.txt")
  lm_plot.4way_cars.txt <- list(
    opt = c("$ts", "[1] FALSE", "", "$pval.BP", "[1] TRUE",
            "", "$pval.DW", "[1] TRUE", "", "$pval.SW", "[1] TRUE", "", "$pred.intvl",
            "[1] TRUE", ""),
    parm = c("$pts", "$pts$symsz", "[1] 1.5", "",
             "$pts$lblsz", "[1] 2.25", "", "$pts$shape", "$pts$shape$reg",
             ...
             "", "", "$var", "$var$lim", "           x        y", "min  9.19249 -3.94098",
             "max 29.31238  5.85379", "", "$var$BP", "", "\tstudentized Breusch-Pagan test",
             "", "data:  mdl", "BP = 0.8807, df = 2, p-value = 0.644", "",
             "", "", "$qq", "$qq$lim", "           x        y", "min -2.15387 -3.94098",
             "max  2.15387  5.85379", "", "$qq$qqlin", "$qq$qqlin$probs",
             "[1] 0.25 0.75", "", "$qq$qqlin$theory", "[1] -0.67449  0.67449",
             "", "$qq$qqlin$resid", "[1] -1.60022  1.04986", "", "$qq$qqlin$slope",
             "[1] 1.96451", "", "$qq$qqlin$int", "[1] -0.275183", "", "",
             "$qq$SW", "", "\tShapiro-Wilk normality test", "", "data:  df$.resid",
             "W = 0.9279, p-value = 0.0343", "", "", ""),
    df = c(
      "                                    .id .sequence .obs    .fits     .resid",
      ...
      "Volvo 142E              reg"))
  expect_equal(lm_plot.4way_cars.out, lm_plot.4way_cars.txt)
})
